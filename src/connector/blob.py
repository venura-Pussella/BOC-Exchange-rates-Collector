...
def backup_for_reference(html_string: str, screenshot: bytes):
    # Initialize BlobServiceClient
    blob_service_client = BlobServiceClient.from_connection_string(connect_str)
    ...
    if len(sorted_blob_list) > 2*config.backup_pairs_to_keep:
        blobs_to_remove = sorted_blob_list[2*config.backup_pairs_to_keep:]
        for blob in blobs_to_remove:
            blob_client = container_client.get_blob_client(blob)
            blob_client.delete_blob()
    # upload the new backups
    filename = config.backup_base_filename + str(datetime.now())
    container_client.upload_blob(f'{filename}.png', screenshot)
